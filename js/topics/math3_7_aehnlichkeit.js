 ...
 document.querySelectorAll('[data-dilation-lab]').forEach(zone=>{
  ...
  const graph=()=>{
   ...
   for(const {p,names}of groups.values()){
    ...
    const candidates=[7,28,49,70].flatMap(gap=>[[sx-width/2,sy-height-gap],[sx+gap,sy-height/2],[sx-width-gap,sy-height/2],[sx-width/2,sy+gap],[sx+gap,sy-height-gap],[sx+gap,sy+gap],[sx-width-gap,sy-height-gap],[sx-width-gap,sy+gap]]);
    const chosen=candidates.find(([x,y])=>x>=48&&x+width<=470&&y>=27&&y+height<=328&&!occupied.some(r=>x<r.x+r.width&&x+width>r.x&&y<r.y+r.height&&y+height>r.y)&&![...groups.values()].some(g=>{const [px,py]=screen(g.p);return px+5>x&&px-5<x+width&&py+5>y&&py-5<y+height;}))||candidates[0];const [x,y]=chosen;occupied.push({x,y,width,height});const lx=Math.max(x,Math.min(sx,x+width)),ly=Math.max(y,Math.min(sy,y+height)),distance=Math.hypot(lx-sx,ly-sy);if(distance>18)add('line',{x1:sx+(lx-sx)*6/distance,y1:sy+(ly-sy)*6/distance,x2:lx,y2:ly,stroke:'#64748b','stroke-width':1,'data-dilation-label-leader':''});add('rect',{x,y,width,height,fill:'white','fill-opacity':0.94,rx:2,'data-dilation-label-box':''});add('text',{x:x+3,y:y+15,'font-size':15,fill:'#111827','data-dilation-label':''},text);
   }
   drawing.replaceChildren(svg);zone.dataset.revealed=String(revealed);
  };
  const renderTable=()=>{
   table.replaceChildren();if(!revealed)return;const{source,target,k}=state(),t=el('table'),caption=el('caption','Koordinaten der Ausgangs- und Bildfigur'),head=el('thead'),hr=el('tr');for(const text of ['Punkt','Ausgangsfigur','Bildfigur']){const th=el('th',text);th.scope='col';hr.append(th);}head.append(hr);const body=el('tbody');source.forEach((p,i)=>{const tr=el('tr'),th=el('th','ABCD'[i]);th.scope='row';const original=el('td',point(p)),image=el('td',point(target[i]));original.dataset.label='Ausgangsfigur';image.dataset.label='Bildfigur';tr.append(th,original,image);body.append(tr);});t.append(caption,head,body);table.append(t);
