...
(async()=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>window.currentChapterQuiz?.questions.length===39);
  ...
  assert.equal(paths,255);assert.equal(await page.evaluate(()=>JSON.stringify(Object.fromEntries(Object.keys(localStorage).map(k=>[k,localStorage.getItem(k)])))),storage);
  ...
  assert.equal(result.contentRevision,3);assert.equal(result.lastPercent,97);assert.deepEqual(result.reviewQuestionIds,['astro_d9']);
  ...
  assert.equal(await paper.locator('#ws-content > .question-block').count(),39);assert.equal(await paper.locator('[data-astronomy-source-comparison] tbody tr').count(),2);
  assert.equal(await paper.locator('#ws-solutions').isVisible(),false);await paper.locator('#ws-include-solutions').check();assert.equal(await paper.locator('.ws-paper-solution').count(),3);
  ...
  assert.deepEqual(errors,[]);fs.writeFileSync(path.join(output,'astronomy-questions-report.json'),JSON.stringify({createdAt:new Date().toISOString(),browser:browser.version(),practiceAnswerPaths:paths,scope:'All 255 native practice choices, repeatable feedback without storage writes, 39-question check with a relocated final question and exact review section, source table and paper toggles. Factual review and screenshot/PDF inspection are separate.',widths,result,pageErrors:errors},null,2)+'\n');
  console.log('PASS: all 255 native practice choices, editable feedback and stable storage, 97% chapter check with relocated question targeting section 20, widths 320/390/1280, source table and three paper solutions.');
