...
const base=process.env.SCIVERSE_PREVIEW_URL||'http://127.0.0.1:4173',out=path.resolve(process.env.SCIVERSE_BROWSER_REPORT_DIR||path.join(__dirname,'../../browser-qa/communication-decisions')),topic='dgb7_kommunikation';
const keys=require('./fixtures/dgb7_communication_keys.json'),cases={cloud:[1,2],account:[0,2],identity:[1,0]};
(async()=>{fs.mkdirSync(out,{recursive:true});const browser=await chromium.launch({headless:true}),errors=[];let decisions=0,answers=0;try{
 ...
 for(const dark of [false,true])for(const width of [320,390,1280]){
  ...
  for(const [name,answers]of Object.entries(cases)){
   const lab=p.locator('[data-communication-lab="'+name+'"]'),select=lab.locator('select'),status=lab.locator('[role=status]');
  ...
  if(width===320)await p.locator('[data-communication-chain]').screenshot({path:path.join(out,'system-'+(dark?'dark':'light')+'.png')});
 }
 ...
 const paper=await browser.newPage();paper.on('pageerror',e=>errors.push(e.message));await paper.goto(base+'/topics/worksheet.html?topic='+topic);await paper.waitForFunction(()=>!document.getElementById('ws-print').disabled);assert.equal(await paper.locator('#ws-solutions').isVisible(),false);assert.equal(await paper.locator('[data-communication-paper] .ai-paper-case').count(),6);await paper.locator('#ws-include-solutions').check();await paper.pdf({path:path.join(out,'dgb7_kommunikation-solutions.pdf'),format:'A4',printBackground:true,preferCSSPageSize:true,margin:{top:'15mm',right:'15mm',bottom:'15mm',left:'15mm'}});
