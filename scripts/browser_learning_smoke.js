...
(async()=>{
 ...
 try{
  ...
  const quizIds=await frame.locator('#sections-container').evaluate(()=>currentChapterQuiz.questions.map(q=>q.id));assert.equal(quizIds.length,13);
  for(const [i,id]of quizIds.entries())await frame.locator('input[name="chapter_q_'+i+'"][value="'+(['dgb7_produktion_q1','dgb7_settings_scope','dgb7_media_scope','dgb7_config_scope'].includes(id)?1:['dgb7_settings_evidence','dgb7_media_framing','dgb7_config_mute'].includes(id)?2:0)+'"]').check();
  await frame.locator('.chapter-submit-btn').click();
  await frame.locator('#chapter-quiz-result').getByText(/92\s*%/).first().waitFor();
  ...
  assert.ok(share.includes('plan='));assert.ok(!share.includes('92'));assert.ok(!share.includes('reviewQuestionIds'));
