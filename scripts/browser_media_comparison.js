...
(async()=>{
 ...
 try{
  ...
  await chapter.getByRole('button',{name:'Zum Kapitelcheck',exact:true}).click();const questions=await chapter.evaluate(()=>currentChapterQuiz.questions.map(q=>q.id));assert.equal(questions.length,10);
  for(const [i,id]of questions.entries()){const value=id==='dgb7_media_scope'?0:id==='dgb7_settings_scope'?1:['dgb7_settings_evidence','dgb7_media_framing'].includes(id)?2:0;await chapter.locator('input[name="chapter_q_'+i+'"][value="'+value+'"]').check();}
  await chapter.locator('.chapter-submit-btn').click();const result=await chapter.evaluate(()=>JSON.parse(localStorage.getItem('sciverse_chapter_quiz_results')).dgb7_produktion);assert.equal(result.lastPercent,90);assert.equal(result.contentRevision,3);assert.deepEqual(result.reviewQuestionIds,['dgb7_media_scope']);
  ...
  await context.close();console.log('PASS: 27 media states, actual bar proportions, three downloaded/opened files, preserved source/data, keyboard and failed-download recovery, 90% ten-question check with exact media review section, paper tasks and solutions.');
