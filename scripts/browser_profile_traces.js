...
const keys=require('./fixtures/dgb8_information_keys.json');
(async()=>{fs.mkdirSync(out,{recursive:true});const browser=await chromium.launch({headless:true}),errors=[];try{
 ...
 for(const [index,id]of questions.entries())for(let choice=0;choice<3;choice++){
  ...
  await page.locator(`input[name="chapter_q_${index}"][value="${choice}"]`).check();await page.locator('#chapter-quiz-panel .chapter-submit-btn').click();const result=await page.evaluate(()=>JSON.parse(localStorage.getItem('sciverse_chapter_quiz_results')).dgb8_information);assert.equal(result.lastPercent,choice===keys[id]?100:88);assert.deepEqual(result.reviewQuestionIds,choice===keys[id]?[]:[id]);answers++;
 }
 ...
 assert.deepEqual(errors,[]);assert.equal(answers,24);fs.writeFileSync(path.join(out,'report.json'),JSON.stringify({createdAt:new Date().toISOString(),browser:browser.version(),states,answers,reset:true,storageUnchanged:true,errors},null,2));console.log('PASS: 96 native source/context/width/theme states, independent counts, disclosure/checkbox keyboard use, reset/focus/storage, 24 assessed answers and worksheet export.');
