...
(async()=>{
 ...
 try{
  ...
  const paper=await context.newPage();await paper.goto(base+'/topics/worksheet.html?topic=dgb7_produktion');await paper.waitForFunction(()=>!document.getElementById('ws-print').disabled);assert.equal(await paper.locator('[data-reading-settings-tasks]>li').count(),6);assert.equal(await paper.locator('[data-reading-settings-protocol] tbody tr').count(),4);assert.equal(await paper.locator('#ws-content>.question-block').count(),13);assert.equal(await paper.locator('#ws-solutions').isVisible(),false);await paper.locator('#ws-include-solutions').check();await paper.pdf({path:path.join(output,'reading-chapter.pdf'),format:'A4',printBackground:true,margin:{top:'15mm',bottom:'15mm',left:'15mm',right:'15mm'}});
  ...
  await context.close();console.log('PASS: 54 rendered settings states, stable text, contrast, native keyboard and storage cycle, unavailable storage, spacing override, chapter links/download and thirteen-question paper export.');
