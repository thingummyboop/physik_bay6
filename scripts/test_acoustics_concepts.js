...
(async()=>{
 ...
 await w.renderTopic();assert.equal(d.querySelectorAll('.chapter-question').length,22);
 ...
 for(const q of protection){
  ...
  for(let choice=0;choice<q.answers.length;choice++){
   ...
   assert.equal(result.lastPercent,Math.round(100*(22-(q.answers[choice].correct?0:1))/22));
 ...
 dom.window.close();console.log('PASS: 22 core acoustics questions including all nine hearing-protection answer paths, revised concepts and results, six reversible resonance settings, no automatic glass break or fictitious Hertz label.');
