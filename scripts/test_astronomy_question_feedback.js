...
(async()=>{
 ...
 assert.equal(questions.length,85);assert.equal(new Set(ids).size,85);assert.equal(d.querySelectorAll('.practice-box').length,85);
 assert.equal(chapter.diplom,undefined);assert.equal(w.currentChapterQuiz.questions.length,39);assert.equal(questions.filter(q=>q.practiceOnly).length,46);
 ...
 for(const [index,q]of w.currentChapterQuiz.questions.entries()){
  for(let choice=0;choice<3;choice++){
   ...
   assert.equal(result.lastPercent,q.answers[choice].correct?100:97);assert.equal(result.contentRevision,3);
 ...
 assert.equal(pd.querySelectorAll('#ws-content > .question-block').length,39);
 ...
 paper.window.close();console.log('PASS: all 255 practice answer/feedback paths across 85 questions, repeatable choices without storage writes, all 117 assessed answer paths, 39 exact review targets including 10 relocated questions, revision 3, dated source table and separate paper solutions. Factual truth requires the documented editorial review.');
