...
// Orientation's expanded pool and moved history section are covered by test_orientation_models.js.
(async()=>{let paths=0;for(const [id,qid,section]of [['dgb5_kommunikation','dgb5_revision_evidence',2],['dgb5_handeln','dgb5_saved_evidence',0]]){
 ...
 }assert.equal(paths,6);console.log('PASS: two unchanged DGB transfer questions, six answer paths, section review routes and invalidated prior results; expanded orientation has its own complete test.');})().catch(e=>{console.error(e);process.exitCode=1;});
