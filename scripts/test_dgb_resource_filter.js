...
(async()=>{
 ...
 const qs=w.currentChapterQuiz.questions;assert.equal(qs.length,4);for(let index=0;index<qs.length;index++)for(let answer=0;answer<3;answer++){qs.forEach((q,i)=>d.querySelector('input[name="chapter_q_'+i+'"][value="'+(i===index?answer:q.answers.findIndex(a=>a.correct))+'"]').checked=true);w.submitChapterQuiz();const result=JSON.parse(w.localStorage.getItem('sciverse_chapter_quiz_results')).dgb6_information;assert.equal(result.lastPercent,answer===0?100:75);if(answer!==0)assert.deepEqual(result.reviewQuestionIds,[qs[index].id]);assert.ok(d.querySelector('#chapter-quiz-result').textContent.includes(qs[index].answers[answer].feedback));}
 ...
 dom.window.close();console.log('PASS: 330 filter/sort cases, full source preservation, filter removal, focus/labels, repeated init, unchanged quiz storage and all twelve assessment paths.');
