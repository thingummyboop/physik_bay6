...
(async()=>{
 ...
 assert.equal(slider.dir,'ltr');assert.equal(d.getElementById('moon-lit').dir,'ltr');
 assert.equal(d.getElementById('sections-container').dir,'rtl');
 for(const [angle,percent,name] of [[0,0,'المحاق'],[45,15,'القمر المتزايد'],[90,50,'التربيع الأول'],[135,85,'القمر المتزايد'],[180,100,'البدر'],[225,85,'القمر المتناقص'],[270,50,'التربيع الأخير'],[315,15,'القمر المتناقص'],[360,0,'المحاق']]){slider.value=angle;slider.dispatchEvent(new w.Event('input'));assert.equal(d.getElementById('moon-lit').value,percent);assert.ok(d.getElementById('moon-explanation').textContent.includes(name));assert.ok(slider.getAttribute('aria-valuetext').includes(name));assert.ok(d.getElementById('moon-explanation').textContent.includes('\u2066'+angle+'°\u2069'));assert.ok(d.getElementById('moon-explanation').textContent.includes('\u2066'+percent+'%\u2069'));assert.ok(d.getElementById('moon-disc').getAttribute('aria-label').includes('من دون ظل الأرض'));}
 assert.match(d.getElementById('sections-container').textContent,/لا يعرض هذا النموذج الكسوف والخسوف/);
 ...
 dom.window.close();console.log('PASS: Arabic Earth/Moon/Sun: current selection, eleven equivalent questions and every answer feedback, phase controls and disc descriptions.');
