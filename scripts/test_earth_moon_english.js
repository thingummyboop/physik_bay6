...
(async()=>{
 ...
 for(const [angle,percent,name] of [[0,0,'New Moon'],[90,50,'First quarter'],[180,100,'Full Moon'],[270,50,'Last quarter'],[360,0,'New Moon']]){slider.value=angle;slider.dispatchEvent(new w.Event('input'));assert.equal(d.getElementById('moon-lit').value,percent);assert.ok(d.getElementById('moon-explanation').textContent.includes(name));assert.ok(d.getElementById('moon-disc').getAttribute('aria-label').includes('without Earth’s shadow'));}
 assert.match(d.getElementById('sections-container').textContent,/does not show eclipses/);
 ...
 dom.window.close();console.log('PASS: English Earth/Moon/Sun: current selection, eleven equivalent questions and every answer feedback, phase controls and disc descriptions.');
