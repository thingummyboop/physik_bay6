...
(async()=>{
 ...
 for(const [angle,percent,name] of [[0,0,'Mlad Mesec'],[90,50,'Prva četvrt'],[180,100,'Pun Mesec'],[270,50,'Poslednja četvrt'],[360,0,'Mlad Mesec']]){slider.value=angle;slider.dispatchEvent(new w.Event('input'));assert.equal(d.getElementById('moon-lit').value,percent);assert.ok(d.getElementById('moon-explanation').textContent.includes(name));assert.ok(d.getElementById('moon-disc').getAttribute('aria-label').includes('bez Zemljine senke'));}
 assert.match(d.getElementById('sections-container').textContent,/model ne prikazuje pomračenja/);
 ...
 dom.window.close();console.log('PASS: Serbian Earth/Moon/Sun: current selection, eleven equivalent questions and every answer feedback, phase controls and disc descriptions.');
