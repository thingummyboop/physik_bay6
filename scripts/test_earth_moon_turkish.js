...
(async()=>{
 ...
 for(const [angle,percent,name] of [[0,0,'Yeni ay'],[90,50,'İlk dördün'],[180,100,'Dolunay'],[270,50,'Son dördün'],[360,0,'Yeni ay']]){slider.value=angle;slider.dispatchEvent(new w.Event('input'));assert.equal(d.getElementById('moon-lit').value,percent);assert.ok(d.getElementById('moon-explanation').textContent.includes(name));assert.ok(d.getElementById('moon-disc').getAttribute('aria-label').includes('Dünya gölgesi olmadan'));}
 assert.match(d.getElementById('sections-container').textContent,/model tutulmaları göstermez/);
 ...
 dom.window.close();console.log('PASS: Turkish Earth/Moon/Sun: current selection, eleven equivalent questions and every answer feedback, phase controls and disc descriptions.');
