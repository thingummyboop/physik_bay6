...
(async()=>{
 ...
 for(let mask=0;mask<16;mask++){
  ...
  assert.ok(status.textContent.includes('Harcama: '+spending+' €. Kalan: '+left+' €'));
  assert.ok(status.textContent.includes(left<0?'tutarını aşıyor':left<5?'payı ayırmıyor':'en az 5 €'));
  assert.doesNotMatch(status.textContent,/Ausgaben|Rücklage|verfügbaren|überschreitet/);
 }
 w.initGeoExperiments();inputs.forEach(i=>i.checked=false);inputs[0].dispatchEvent(new w.Event('change'));assert.match(status.textContent,/Kalan: 20 €/);
 ...
 dom.window.close();console.log('PASS: Turkish economics chapter: all 16 budget combinations, four equivalent questions, matching workshop and complete assessment.');
