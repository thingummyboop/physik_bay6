...
assert.equal(w.currentChapterQuiz.questions.find(q=>q.id==='q_ohm').sectionIndex,5);assert.equal(w.currentChapterQuiz.questions.find(q=>q.id==='elektrizitaet_s4_q0').sectionIndex,8);
...
for(const q of protection){
 ...
 for(let choice=0;choice<q.answers.length;choice++){
  ...
  assert.equal(result.lastPercent,Math.round(100*(28-(q.answers[choice].correct?0:1))/28));
...
assert.match(d.querySelector('#ohmFeedback').innerText,/keine Vorhersage/);dom.window.close();console.log('PASS: corrected electricity concepts, 28 assessed questions including 21 protection/device answer paths and twelve sensor-lamp states, eight circuit topologies/switch states verified with independent path traversal, obsolete inline final removed, revision migration, five Ohm scenarios and accessible values.');})().catch(e=>{console.error(e);process.exitCode=1;});
