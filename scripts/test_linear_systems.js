...
assert.equal(d.querySelectorAll('.practice-box').length,8);assert.ok(d.getElementById('ggb-funktionen'));assert.match(d.getElementById('topic-title').textContent,/Gleichungssysteme/);
...
dom.window.close();console.log('PASS: actual expanded chapter, 8 practice questions, retained graph explorer; 625 coefficient combinations checked in both equations, all three solution cases, outside-window point, invalid input and numerical limit.');
