...
(async()=>{
 ...
 const table=d.querySelector('#sections-container table');assert.ok(table);assert.equal(table.querySelectorAll('tbody tr').length,4);assert.equal(table.querySelectorAll('th[scope="col"]').length,4);assert.match(table.textContent,/Mira, Beč/);assert.match(table.textContent,/Amani, Najrobi/);assert.match(table.textContent,/Jo, Najrobi/);assert.match(table.textContent,/Sol, okolina Lime/);assert.match(d.getElementById('sections-container').textContent,/četiri potpuno izmišljena učenika/);
 ...
 dom.window.close();console.log('PASS: Serbian living situations chapter: four case studies, eight equivalent questions, matching workshop and complete assessment.');
