...
(async()=>{
 ...
 const table=d.querySelector('#sections-container table');assert.ok(table);assert.equal(table.querySelectorAll('tbody tr').length,4);assert.equal(table.querySelectorAll('th[scope="col"]').length,4);assert.match(table.textContent,/Mira, Viyana/);assert.match(table.textContent,/Amani, Nairobi/);assert.match(table.textContent,/Jo, Nairobi/);assert.match(table.textContent,/Sol, Lima çevresi/);assert.match(d.getElementById('sections-container').textContent,/tamamen kurgusal dört öğrenci/);
 ...
 dom.window.close();console.log('PASS: Turkish living situations chapter: four case studies, eight equivalent questions, matching workshop and complete assessment.');
