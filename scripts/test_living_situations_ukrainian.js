...
(async()=>{
 ...
 const table=d.querySelector('#sections-container table');assert.ok(table);assert.equal(table.querySelectorAll('tbody tr').length,4);assert.equal(table.querySelectorAll('th[scope="col"]').length,4);assert.match(table.textContent,/Міра, Відень/);assert.match(table.textContent,/Амані, Найробі/);assert.match(table.textContent,/Джо, Найробі/);assert.match(table.textContent,/Соль, околиці Ліми/);assert.match(d.getElementById('sections-container').textContent,/чотирьох цілком вигаданих учнів/);
 ...
 dom.window.close();console.log('PASS: Ukrainian living situations chapter: four case studies, eight equivalent questions, matching workshop and complete assessment.');
