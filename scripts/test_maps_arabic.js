...
(async()=>{
 ...
 assert.match(status.textContent,/الموقع ⁦A5⁩/);move('w');assert.match(status.textContent,/حدود المخطط النموذجي/);assert.match(status.textContent,/عدد الخطوات: 0/);
 move('e');move('e');move('n');move('n');assert.match(status.textContent,/C3⁩ مغلقة/);assert.match(status.textContent,/الموقع ⁦C4⁩. عدد الخطوات: 3/);
 reset();w.initGeoExperiments();for(let i=0;i<4;i++)move('n');for(let i=0;i<4;i++)move('e');assert.match(status.textContent,/800 متر/);assert.match(status.textContent,/وصلت إلى الهدف/);assert.match(zone.querySelector('[data-grid]').getAttribute('aria-label'),/الموقع ⁦E1⁩/);assert.doesNotMatch(zone.textContent,/Standort|Schritte|gesperrt|Ziel erreicht/);reset();assert.match(status.textContent,/الموقع ⁦A5⁩. عدد الخطوات: 0/);
 assert.equal(zone.querySelector('[data-grid]').dir,'ltr');
 assert.equal(zone.querySelector('[data-grid]').children[0].textContent,'A1');
 assert.ok(zone.querySelector('[data-grid]').children[4].textContent.startsWith('E1'));
 assert.ok(d.querySelector('bdi[dir="ltr"]'));
 ...
 dom.window.close();console.log('PASS: Arabic maps chapter: translated route controls/status, map boundary, blocked cell, shortest route, reset, four assessments and workshop.');
