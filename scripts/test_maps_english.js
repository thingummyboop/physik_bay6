...
(async()=>{
 ...
 assert.match(status.textContent,/Position A5/);move('w');assert.match(status.textContent,/edge of the model map/);assert.match(status.textContent,/0 steps/);
 move('e');move('e');move('n');move('n');assert.match(status.textContent,/C3 is closed/);assert.match(status.textContent,/Position C4. 3 steps/);
 reset();w.initGeoExperiments();for(let i=0;i<4;i++)move('n');for(let i=0;i<4;i++)move('e');assert.match(status.textContent,/800 metres/);assert.match(status.textContent,/Destination reached/);assert.match(zone.querySelector('[data-grid]').getAttribute('aria-label'),/Position E1/);assert.doesNotMatch(zone.textContent,/Standort|Schritte|gesperrt|Ziel erreicht/);reset();assert.match(status.textContent,/Position A5. 0 steps/);
 ...
 dom.window.close();console.log('PASS: English maps chapter: translated route controls/status, map boundary, blocked cell, shortest route, reset, four assessments and workshop.');
