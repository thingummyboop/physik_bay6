...
(async()=>{
 ...
 [...workshop.querySelectorAll('button')].find(b=>b.textContent==='Перевірити відповідності').click();assert.match(workshop.textContent,/Правильних відповідностей: 3 із 3/);
 ...
 assert.match(status.textContent,/Позиція A5/);move('w');assert.match(status.textContent,/межа модельного плану/);assert.match(status.textContent,/Кількість кроків: 0/);
 move('e');move('e');move('n');move('n');assert.match(status.textContent,/C3 закрито/);assert.match(status.textContent,/Позиція C4. Кількість кроків: 3/);
 reset();w.initGeoExperiments();for(let i=0;i<4;i++)move('n');for(let i=0;i<4;i++)move('e');assert.match(status.textContent,/800 м/);assert.match(status.textContent,/Цілі досягнуто/);assert.match(zone.querySelector('[data-grid]').getAttribute('aria-label'),/Позиція E1/);assert.doesNotMatch(zone.textContent,/Standort|Schritte|gesperrt|Ziel erreicht/);reset();assert.match(status.textContent,/Позиція A5. Кількість кроків: 0/);
 ...
 dom.window.close();console.log('PASS: Ukrainian maps chapter: translated route controls/status, map boundary, blocked cell, shortest route, reset, four assessments and workshop.');
