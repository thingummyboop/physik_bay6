...
(async()=>{
 ...
 for(const [i,q] of questions.entries()){
  ...
  for(let choice=0;choice<q.answers.length;choice++){
   ...
   const result=JSON.parse(w.localStorage.getItem('sciverse_chapter_quiz_results'))[id];assert.equal(result.lastPercent,choice===correctIndex(q.id)?100:92);assert.equal(result.contentRevision,4);
