...
(async()=>{
 ...
 for(const id of ids){
  ...
  if(id==='math1_4_mult_div'){
   ...
   let paths=0;for(const [i,q]of w.currentChapterQuiz.questions.entries())if(q.id.startsWith('division_')||q.id.startsWith('written_')||q.id==='math1_4_q6')for(let a=0;a<q.answers.length;a++){assert.equal(q.sectionIndex,q.id==='written_multiplication_place'?0:q.id==='math1_4_q6'?2:1);w.currentChapterQuiz.questions.forEach((item,j)=>d.querySelector('input[name="chapter_q_'+j+'"][value="'+(j===i?a:item.answers.findIndex(x=>x.correct))+'"]').checked=true);w.submitChapterQuiz();const n=w.currentChapterQuiz.questions.length;assert.equal(JSON.parse(w.localStorage.getItem('sciverse_chapter_quiz_results'))[id].lastPercent,Math.round(100*(n-(q.answers[a].correct?0:1))/n));assert.ok(d.getElementById('chapter-quiz-result').textContent.includes(q.answers[a].feedback));paths++;}assert.equal(paths,18);assert.equal(w.currentChapterResult(id,{contentRevision:1,passed:true,bestPercent:100}).passed,false);
  }
