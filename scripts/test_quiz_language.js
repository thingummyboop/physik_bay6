...
(async()=>{for(const language of Object.keys(expected))for(const scenario of ['passed','failed','storage-failed']){
 ...
 for(const f of ['curriculum','chapter-revisions','common','core-learning','language-workshop','renderer'])w.eval(read('js/'+f+'.js'));w.eval('globalPhysikScore=0');
 // Current content may fall back to German; quiz controls remain localized.
 await w.renderTopic();
