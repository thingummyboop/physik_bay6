...
const questions=data.math4_1_reelle_zahlen.sections.flatMap(s=>s.quizzes);assert.equal(questions.length,7);for(const q of questions){assert.ok(d.body.textContent.includes(q.question),q.id+": "+q.question);assert.equal(q.answers.filter(a=>a.correct).length,1);}
...
assert.equal(w.currentChapterResult('math4_1_reelle_zahlen',{passed:true,bestPercent:100}).passed,false);dom.window.close();console.log('PASS: real-number chapter renders seven questions and guides; all 1001 approximation steps and root input checked.');
