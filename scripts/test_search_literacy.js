...
(async()=>{
 ...
 await w.renderTopic();assert.equal(w.currentChapterQuiz.questions.length,7);assert.equal(d.querySelectorAll("[data-file-practice] li").length,7);assert.equal(d.querySelectorAll("[data-file-protocol] tbody tr").length,2);
 ...
 let count=0;for(const [i,q]of w.currentChapterQuiz.questions.entries())if(q.id.startsWith('dgb5_search_')){
  ...
  for(let choice=0;choice<q.answers.length;choice++){
   ...
   w.submitChapterQuiz();assert.equal(JSON.parse(w.localStorage.getItem('sciverse_chapter_quiz_results'))[id].lastPercent,q.answers[choice].correct?100:86);
