...
for(const [id,topic]of Object.entries(data).filter(([,c])=>c.workshop)){
 ...
 // This suite checks worksheet content and privacy. Actual MathJax output is covered by browser_print_math.js.
 ...
 toggle.checked=true;toggle.dispatchEvent(new w.Event('change'));assert.equal(material.hidden,false);if(solutions)assert.equal(solutions.hidden,false);assert.equal(d.querySelector('#ws-print').disabled,false,id+' '+d.querySelector('#ws-math-status').textContent);count++;dom.window.close();
}
